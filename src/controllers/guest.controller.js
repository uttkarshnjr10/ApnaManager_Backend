...
const checkWatchlistAndNotify = async (guest, hotel) => {
  try {
    ...
    logger.warn(
      `WATCHLIST MATCH: Guest ${guest.primaryGuest.name} (ID: ${idNumber}) matched watchlist item (Reason: ${match.reason})`
    );
    ...
    const populatedAlert = await newAlert.populate(
      'guest',
      'primaryGuest.name idNumber stayDetails.roomNumber'
    );
...
const registerGuest = asyncHandler(async (req, res) => {
  ...
  console.log(
    '[DEBUG] registerGuest - files:',
    files ? `${files.length} files received` : 'no files'
  );
  if (files.length > 0) {
    console.log(
      '[DEBUG] File details:',
  ...
  console.log('[DEBUG] Calling uploadToCloudinary for each file...');
  const uploadResults = await Promise.all(
    files.map((file) => {
      console.log('[DEBUG] Mapping file with fieldname:', file.fieldname);
      return uploadToCloudinary(file, 'guest-guard');
    })
  );

  console.log('[DEBUG] uploadResults:', uploadResults ? `${uploadResults.length} results` : 'null');
