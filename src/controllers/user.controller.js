...
// Initialize Cache (StdTTL: 3600 seconds = 1 Hour)
// This prevents API misuse. Even if 100 people click "Get Report", Gemini is called ONCE per hour.
  ...
  if (role === 'Hotel') {
    ...
    if (user) {
      try {
        ...
        logger.error(
          `Could not update inquiry for new user ${user.email}: ${inquiryError.message}`
        );
      }
    }
  } else if (role === 'Police') {
...
const getAIDailyReport = asyncHandler(async (req, res) => {
  const role = req.user.role || 'Hotel'; // Fallback
  const userId = req.user._id;

  // 1. CACHE KEY MUST BE UNIQUE PER USER (Safety First!)
  // If we just used "daily_report", Admin would see Hotel's cached report!
  const cacheKey = `ai_report_${userId}`;

  const cachedReport = reportCache.get(cacheKey);
  if (cachedReport) {
    return res.status(200).json(new ApiResponse(200, { summary: cachedReport, source: 'cache' }));
  }

  // 2. DEFINE DATE RANGE (Today)
  ...
  if (role === 'Regional Admin') {
    // ADMIN: Sees EVERYTHING (Aggregate)
    const guestsToday = await Guest.find({
      registrationTimestamp: { $gte: startOfDay, $lte: endOfDay },
    }).select('primaryGuest stayDetails');

    // Count Police Activity (Searches) from AccessLogs
    const policeSearches = await AccessLog.countDocuments({
      timestamp: { $gte: startOfDay, $lte: endOfDay },
      userModel: 'Police',
      action: { $regex: /search/i }, // Assuming 'search' is in the action name
    });

    statsPayload = {
      totalGuests: guestsToday.length,
      foreignNationals: guestsToday.filter((g) => g.primaryGuest.nationality !== 'Indian').length,
      topCities: getTopItems(guestsToday.map((g) => g.primaryGuest.address.city)),
      topPurposes: getTopItems(guestsToday.map((g) => g.stayDetails.purposeOfVisit)),
      policeSearches: policeSearches,
    };
  } else if (role === 'Hotel') {
    // HOTEL: Sees ONLY THEIR OWN DATA
    const guestsToday = await Guest.find({
      hotel: userId, // <--- CRITICAL FILTER
      registrationTimestamp: { $gte: startOfDay, $lte: endOfDay },
    }).select('primaryGuest stayDetails');
    ...
    // POLICE (or others): Feature Disabled
    throw new ApiError(403, 'AI Reporting is not enabled for your role yet.');
  }

  // 4. GENERATE & CACHE
  // Pass 'role' to AI so it knows which persona to use
  ...
  res.status(200).json(new ApiResponse(200, { summary, source: 'live' }));
});
  ...
  if (user) {
    ...
    if (role === 'Hotel' && user.email) {
      try {
        ...
        if (updatedInquiry) {
          logger.info(
            `Associated inquiry for ${user.email} marked as 'Pending' (Rejected equivalent).`
          );
        }
      } catch (inquiryError) {
