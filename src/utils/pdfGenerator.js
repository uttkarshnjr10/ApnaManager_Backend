...
const drawFooter = (doc, guest) => {
  ...
  doc
    ...
    .text('This is a system-generated receipt powered by ApnaManager.', {
      align: 'center',
      width: CONFIG.layout.contentWidth,
    });
};
...
const renderStayDetails = (doc, guest) => {
  ...
  const checkIn = new Date(stayDetails.checkIn).toLocaleString('en-IN', {
    dateStyle: 'medium',
    timeStyle: 'short',
  });
  const checkOut = new Date(stayDetails.expectedCheckout).toLocaleString('en-IN', {
    dateStyle: 'medium',
    timeStyle: 'short',
  });
